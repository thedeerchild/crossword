import { JsonWrapper } from '$lib/types/jsonv';
import { z } from 'zod';
import { flipDirection, type Direction, type GridCursor } from '../stores/cursor';
...
export class Grid {
	private static wrapper = new JsonWrapper(
		'grid',
		z
			.object({
				grid: z.string(),
	...
	static fromJSON(json: string) {
		const data = this.wrapper.fromJSON(json);
		return new Grid(Grid.fromString(data.grid).squares, data.width);
	}
	 ...
	 */
	getCurrentWord(idx: number): {
		[key in Direction]: GridWord | null;
	} {
